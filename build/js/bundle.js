 ...
 * @return 坐标组成的字符串
 */
function offsetCenter(arr2d) {
    // 复制一份
    var newArr = arr2d.map(function (item) {
        return item;
    });
    // 形心
    var centroid = centerXY(arr2d);
    var len = arr2d.length;
    ...
    for (var i = 1; i < len; i++) {
        ...
        var lang = Math.sqrt(Math.pow(A[0] - B[0], 2) + Math.pow(A[1] - B[1], 2));
        if (lang >= 120) {
            // AB的中点
...
exports.createModuleBat = createModuleBat;
exports.createBats = createBats;
...
function createModuleBat(data) {
    data.dependencies.forEach(function (item) {
        ...
        if ($('#' + info.id).length === 0) {
            $('#content .module').append((0, _index.onlyOutputBattery)(info));
        }
    });
}
 ...
 * 根据数据生成bat
 */
function createBats(obj, posArr) {
    for (var key in obj) {
        var element = obj[key];
        if ((typeof element === 'undefined' ? 'undefined' : _typeof(element)) === 'object') {
            ...
            if ($('#' + info.id).length === 0) {
                if (!element.pos.x) {
                    element.pos.x = (0, _tools.randomPos)(element.dir).x;
                }
                if (!element.pos.y) {
                    element.pos.y = (0, _tools.randomPos)(element.dir).y;
                }
                var x = element.pos.x;
                var y = element.pos.y;
                $((0, _index.battery)(info)).css({ top: y, left: x }).appendTo($('#content'));
...
function createRelations(data) {
    ...
    var dev = data.dependencies;
    for (var key in relations) {
        var element = relations[key];
        if ((typeof element === 'undefined' ? 'undefined' : _typeof(element)) === 'object') {
 ...
 * outputId, inputId
 */
function createRelationPath(outputId, inputId) {

    var fromText = $('#' + outputId + ' p.title').text();
    var x1 = $('#' + outputId + ' .output').offset().left + 6;
    var y1 = $('#' + outputId + ' .output').offset().top + 6;

    var toText = $('#' + inputId + ' p.title').text();
    var x2 = $('#' + inputId + ' .input').offset().left + 6;
    var y2 = $('#' + inputId + ' .input').offset().top + 6;

    var path = d3.select('#svg svg').append('path');
    path.attr('output', outputId).attr('start', x1 + ',' + y1);
    var id = outputId + inputId;
    path.attr('input', inputId).attr('end', x2 + ',' + y2).attr('id', id);
    path.attr('d', (0, _tools.curveTo)(x1, y1, x2, y2));
...
function batteryDown(that, event) {
    ...
    var pathsInputM = [];
    if (inputPathsSize > 0) {
        inputPaths.each(function (index, elem) {
    ...
    if (outputPathsSize > 0) {
        outputPaths.each(function (index, elem) {
    ...
    $(document).on('mousemove', function (event) {
        ...
        inputPaths.each(function (index, elem) {
            ...
            elem.setAttribute('d', (0, _tools.curveTo)(inputM[0], inputM[1], inputX, inputY));
            elem.setAttribute('end', inputX + ',' + inputY);
            elem.setAttribute('move', 'true');
        });
        outputPaths.each(function (index, elem) {
            var outputM = pathsOutputM[index].split(',');
            elem.setAttribute('d', (0, _tools.curveTo)(outputX, outputY, outputM[0], outputM[1]));
    ...
    $(document).on('mouseup', '.battery', function () {
        $(document).off('mousemove');
        $(document).off('mouseup');
        $('.selected').removeClass('selected');
        $('path[move]').css({ stroke: '#ccc' }).attr('move', '');
        _socket2.default.emit('position', { batteryId: batteryId, currX: currX, currY: currY });
    });
}
// input mouseup 事件
function inputUp(event) {
    ...
    if (tempX && tempY && currPath) {
        ...
        var fromPath = void 0,
            toPath = void 0;
        var List = (0, _relations.getRelationData)().relations;
        for (var key in List) {
            var elem = List[key];
            if (elem.id === outputId) {
        ...
        var dirList = (0, _relations.getRelationData)().dependencies;
        ...
        if ($('path#' + id).length === 1) {
            _socket2.default.emit('build-relation', { fromA: fromPath, toB: toPath });
            //生成路径文字
            (0, _render.pathText)('#' + id, fromPath.name + ' From ' + toPath.name);
        } else {
            currPath.remove();
        }
...
function outputDown(event) {
    ...
    $(document).on('mousemove', function (event) {
        currPath.attr('d', (0, _tools.curveTo)(tempX, tempY, event.clientX, event.clientY));
    });
...
function editBat(event) {
    ...
    _socket2.default.emit('edit-file', dir + '/' + name);
}

$('body').on('mousedown', 'div.battery', function (event) {
...
$(document).on('mousedown', function (e) {
    ...
    var rect = d3.select('#select svg').append('rect');
...
var interval = 35;
var count = Math.ceil(3000 / interval);
for (var i = 0; i < count; i++) {
    var xline = svg.append('line').attr('x1', interval * (i + 1)).attr('y1', 0).attr('x2', interval * (i + 1)).attr('y2', 3000);
    var yline = svg.append('line').attr('x1', 0).attr('y1', interval * (i + 1)).attr('x2', 3000).attr('y2', interval * (i + 1));
    if (i % 5 === 0) {
...
_socket2.default.emit('init', 'init');
_socket2.default.once('init', function (data) {
    if (data && (typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object') {
        //把数据保存在model中
        (0, _relations.saveRelationData)(data);
        var posArr = [];
        //渲染module
        ...
        //将所有的bat的坐标信息上报服务器
        _socket2.default.emit('position', posArr);
    }
});
